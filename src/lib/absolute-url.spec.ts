...
import { absoluteUrl, isValidUrl, joinPaths } from "./absolute-url"
...
describe("absoluteUrl", () => {
  ...
  describe("with query parameters", () => {
    it("should add query parameters to URL", () => {
      ...
      expect(absoluteUrl("/api/users", { query: { id: "123", active: true } })).toBe(
        "http://localhost:3000/api/users?id=123&active=true"
      )
    })
  ...
  describe("with fragments", () => {
    ...
    it("should encode fragment properly", () => {
      expect(absoluteUrl("/docs", { fragment: "section with spaces" })).toBe(
        "http://localhost:3000/docs#section%20with%20spaces"
      )
    })
